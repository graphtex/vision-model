...
def gen_graph(n, p, f, c, directed = False, show = False, id=0, directory=''):
    ...
    for i in range(len(adjacency)):
        ...
        for j in range(0, end):
            ...
            if adjacency[i][j] == 1:
                ...
                if box_x2 - box_x1 < 0.1:
                    add_x = 0.1 - (box_x2 - box_x1)
                    box_x1 -= add_x
                    box_x2 += add_x
                if box_y2 - box_y1 < 0.1:
                    add_y = 0.1 - (box_y2 - box_y1)
                    box_y1 -= add_y
                    box_y2 += add_y
    ...
    # out['boxes'][:3] = torch.maximum(out['boxes'][:3], torch.ones_like(out['boxes'][:3]) * 0.1)
    # out['boxes'][:4] = torch.maximum(out['boxes'][:4], torch.ones_like(out['boxes'][:3]) * 0.1)
